<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['account:posts:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['account:posts:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="postsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="路径" align="center" prop="filePath" show-overflow-tooltip/>
      <el-table-column label="图片名称" align="center" prop="pictureName" show-overflow-tooltip/>
...
<script>
import { listPosts, getPosts, delPosts, addPosts, updatePosts } from "@/api/account/posts";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/account/posts/importData"
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除帖子编号为"' + keyIds + '"的数据项？').then(function() {
        return delPosts(keyIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('account/posts/export', {
        ...this.queryParams
