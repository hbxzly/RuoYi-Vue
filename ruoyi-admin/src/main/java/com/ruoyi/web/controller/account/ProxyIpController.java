...
public class ProxyIpController extends BaseController
{
    ...
    @GetMapping("/updateIpStatus")
    public AjaxResult updateIpStatus(@RequestParam Long keyId, @RequestParam String status) {
        ProxyIp proxyIp = proxyIpService.selectProxyIpByKeyId(keyId);
        proxyIp.setStatus(status);
        proxyIpService.updateProxyIp(proxyIp);
        return success();
    }
}
